...
int main(int argc, char* argv[]){
  ...
  //try{
    ...
    w.add(RessortForce({-1},{0},{-1,4,0},{3.0,-3.0,0.0},2,1,0,4,3));
    w.add(Ressort({23},{0},{0,1,0},{0,18,0}));
    for(int k(0)  ;k<=nb;k++){
    for(int j(-nb);j<=0 ;j++){
    for(int i(0)  ;i<=nb;i++){
      ...
      w.add(Pendule(       {M_PI-1.00001}, {0},   { 1, 0, 0}, { 0.0      , 0.0+1.0*i, 0.0      },L,m,(0.05*i+0.1)*critique));
      ...
      w.add(PenduleDouble({3.14+0.000000001*i,3.14},{0,0},{1,0,0},{0,1.0*i,0}));
    }}}
